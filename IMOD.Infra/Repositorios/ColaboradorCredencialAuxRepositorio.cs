...
{
    public class ColaboradorCredencialAuxRepositorio : IColaboradorCredencialAuxRepositorio
    {
        private readonly string _connection = CurrentConfig.ConexaoString;
        private readonly IDataBaseAdo _dataBase;
        private readonly IDataWorkerFactory _dataWorkerFactory = new DataWorkerFactory();

        public ColaboradorCredencialAuxRepositorio()
        {
            try
            {
        ...
        public void Alterar(ColaboradorCredencial entity)
        {
            try
            {
                using (var conn = _dataBase.CreateOpenConnection())
                {
                    using (var cmd = _dataBase.UpdateText("ColaboradoresCredenciaisAux", conn))
                    {
                        try
                        {
        ...
        public void Criar(ColaboradorCredencial entity)
        {
            try
            {
                using (var conn = _dataBase.CreateOpenConnection())
                {
                    using (var cmd = _dataBase.InsertText("ColaboradoresCredenciaisAux", conn))
                    {
                        try
                        {
                ...
                using (var conn = _dataBase.CreateOpenConnection())
                {
...

                    {
                        try
                        {
                            ...
                            cmd.CreateParameterSelect(_dataBase.CreateParameter(new ParamSelect("EmpresaContratoID", DbType.Int32, o, 11).Igual()));
        ...
        public void Remover(ColaboradorCredencial entity)
        {
            try
            {
                using (var conn = _dataBase.CreateOpenConnection())
                {
                    using (var cmd = _dataBase.DeleteText("ColaboradoresCredenciaisAux", conn))
                    {
                        try
                        {
        ...
        public ColaboradorCredencial ObterCredencialPeloNumeroCredencial(string numCredencial)
        {
            try
            {
                using (var conn = _dataBase.CreateOpenConnection())
                {
                    using (var cmd = _dataBase.SelectText("ColaboradoresCredenciaisAux", conn))
                ...
                using (var conn = _dataBase.CreateOpenConnection())
                {
                    using (var cmd = _dataBase.SelectText("ColaboradoresCredenciaisView", conn))
                    {
                        try
                        {
                            ...
                            cmd.CreateParameterSelect(_dataBase.CreateParameter(new ParamSelect("ColaboradorEmpresaID", DbType.Int32, o, 6).Igual()));
