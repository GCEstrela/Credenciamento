...
{
    public class VeiculoEmpresaWebRepositorio : IVeiculoEmpresaWebRepositorio
    {
        ...
        public VeiculoEmpresaWebRepositorio()
        {
        ...
        public void Criar(VeiculoEmpresaWeb entity)
        {
            try
            {
                ...
                {
                    using (var cmd = _dataBase.InsertText("VeiculosEmpresasWeb", conn))
                    {

                        cmd.Parameters.Add(_dataBase.CreateParameter(new ParamInsert("VeiculoEmpresaWebID", entity.VeiculoEmpresaWebId, true)));
                        ...
                        entity.VeiculoEmpresaWebId = key;
                        //Gerar matricula
        ...
        public void CriarNumeroMatricula(VeiculoEmpresaWeb entity)
        {
            ...
            {
                var codigo = "";
                if(entity.VeiculoEmpresaId == 0)
                {
                    codigo = entity.VeiculoEmpresaWebId.ToString("N0");
                }
                else
                {
                    codigo = entity.VeiculoEmpresaId.ToString("N0");
                }
        ...
        public VeiculoEmpresaWeb BuscarPelaChave(int id)
        {
            try
            {
                using (var conn = _dataBase.CreateOpenConnection())
                {
                    using (var cmd = _dataBase.SelectText("VeiculosEmpresasWeb", conn))

                    {
                        ...
                        var d1 = reader.MapToList<VeiculoEmpresaWeb>();
        ...
        public void Alterar(VeiculoEmpresaWeb entity)
        {
            try
            {
                using (var conn = _dataBase.CreateOpenConnection())
                {
                    using (var cmd = _dataBase.UpdateText("VeiculosEmpresasWeb", conn))
                    {

                        cmd.Parameters.Add(_dataBase.CreateParameter(new ParamUpdate("VeiculoEmpresaWebID", entity.VeiculoEmpresaWebId, true)));
