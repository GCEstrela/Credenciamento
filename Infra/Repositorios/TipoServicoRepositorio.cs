        ...
        public void Criar(TipoServico entity)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.InsertText("TipoServico", conn))
                {
                    ...
                    {
                        cmd.Parameters.Add(
                            _dataBase.CreateParameter(new ParamInsert("TipoServicoID", entity.TipoServicoId, true)));
                        cmd.Parameters.Add(
        ...
        public TipoServico BuscarPelaChave(int id)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
...

                {
                    ...
                    {
                        cmd.Parameters.Add(
                            _dataBase.CreateParameter(new ParamSelect("TipoServicoId", DbType.Int32, id).Igual()));
        ...
        public void Alterar(TipoServico entity)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.UpdateText("TipoServico", conn))
                {
                    ...
                    {
                        cmd.Parameters.Add(
                            _dataBase.CreateParameter(new ParamUpdate("TipoServicoID", entity.TipoServicoId, true)));
                        cmd.Parameters.Add(
        ...
        public void Remover(TipoServico entity)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.DeleteText("TipoServico", conn))
                {
                    ...
                    {
                        cmd.Parameters.Add(
                            _dataBase.CreateParameter(new ParamDelete("TipoServicoId", entity.TipoServicoId).Igual()));
