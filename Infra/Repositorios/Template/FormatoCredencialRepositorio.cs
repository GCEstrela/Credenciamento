...
namespace IMOD.Infra.Repositorios
{
    public class FormatoCredencialRepositorio :IFormatoCredencialRepositorio 
    {
        ...
        public void Criar(FormatoCredencial entity)
        {
            ...
            {
                using (var cmd = _dataBase.InsertText ("FormatoCredencial", conn))
                {
                    try
                    {
        ...
        public FormatoCredencial BuscarPelaChave(int id)
        {
           using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.SelectText ("FormatoCredencial", conn))
        ...
        public void Alterar(FormatoCredencial entity)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.UpdateText ("FormatoCredencial", conn))
                {
                    try
                    {
                        cmd.Parameters.Add (_dataBase.CreateParameter (new ParamUpdate ("FormatoCredencialId", entity.FormatoCredencialId, true)));
        ...
        public void Remover(FormatoCredencial entity)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.DeleteText ("FormatoCredencial", conn))
                {
                    try
                    {
