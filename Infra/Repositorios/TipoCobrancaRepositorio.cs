        ...
        public void Criar(TipoCobranca entity)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.InsertText("TiposCobrancas", conn))
                {
                    ...
                    {
                        cmd.Parameters.Add(
                            _dataBase.CreateParameter(new ParamInsert("TipoCobrancaId", entity.TipoCobrancaId, true)));
                        cmd.Parameters.Add(
        ...
        public TipoCobranca BuscarPelaChave(int id)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
...

                {
                    ...
                    {
                        cmd.Parameters.Add(
                            _dataBase.CreateParameter(new ParamSelect("TipoCobrancaId", DbType.Int32, id).Igual()));
        ...
        public void Alterar(TipoCobranca entity)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.UpdateText("TiposCobrancas", conn))
                {
                    ...
                    {
                        cmd.Parameters.Add(
                            _dataBase.CreateParameter(new ParamUpdate("TipoCobrancaId", entity.TipoCobrancaId, true)));
                        cmd.Parameters.Add(
        ...
        public void Remover(TipoCobranca entity)
        {
            using (var conn = _dataBase.CreateOpenConnection())
            {
                using (var cmd = _dataBase.DeleteText("TiposCobrancas", conn))
                {
                    ...
                    {
                        cmd.Parameters.Add(
                            _dataBase.CreateParameter(new ParamDelete("TipoCobrancaId", entity.TipoCobrancaId)
                                .Igual()));
